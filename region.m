...
function [columns,activeColumns,cells,prediction,output] = region(dInput,inputConfig,id,columns,cells,nextConfig)
    ...
    for t = 1:n.time
        ...
        if ~isempty(nextConfig)
            [columns2, activeColumns2, cells2, output2] = region(output,nextConfig(:,id),id+1,[],[]);
        end
    end
end