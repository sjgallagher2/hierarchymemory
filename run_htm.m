...
%% Initialize window and variables
    ...
    dProjFile = ''; %this holds the project file name for saving (not saving as)
    dProjLoc = '';
    projSaved = true;
   ...
   %Create config files
    ...
    hFileMenu = uimenu(h.fig,'Label','File');
        ...
        uimenu(hFileMenu,'Label','Save as...','Callback',@cbFSMA);
...
%FILE MENU CALLBACKS
    ...
    function cbFO(hObject,evt)
        ...
        if projSaved == false
            ...
            resp = questdlg('Save before opening new file?');
            if strcmp(resp, 'Yes') == true
                cbFSM(hObject, evt);
            end
        end
        region_data = guidata(h.fig);
        [dProjFile, dProjLoc]= uigetfile({'*.htm', 'HTM Project Files (*.htm)'},'Open Project',region_config(1).lastDir); 
        if dProjFile ~= 0
            region_config(1).lastDir = dProjLoc;
            hClearMem.Enable = 'on';
            hDataSaveDataSeq.Enable = 'on';
            %Open the relevant project data; region_data, region_config and
            %send
            FID = fopen([dProjLoc dProjFile],'r');
            tempPaths = textscan(FID,'%s');
            fclose(FID);

            tempPaths = tempPaths{1};
            A = load(char(tempPaths(1)));
            region_data = A.region_data;
            A = load(char(tempPaths(2)));
            region_config = A.region_config;
            A = load(char(tempPaths(3)));
            send = A.send;
            %Set the home screen display image etc
            if ~isempty(send)
                hRunItem.Enable = 'on';
                hClearMem.Enable = 'on';
    ...
    function cbFSM(hObject,evt)
        %Save memory
        region_data = guidata(h.fig);
        if strcmp(dProjFile, '') == true
            [dProjFile, dProjLoc] = uiputfile({'*.htm', 'HTM Project (*.htm)'}, 'Save Project',region_config(1).lastDir); 
        end
        if dProjFile ~= 0
            %Create a project folder with .mat files for structures
            dFolder = [dProjLoc dProjFile(1:end-4),'_data'];
            if ~exist(dFolder)
                mkdir(dFolder);
            end
            save([dFolder,'\\RegionData.mat'],'region_data');
            save([dFolder,'\\RegionConfig.mat'],'region_config');
            save([dFolder,'\\InputData.mat'],'send');
            
            %Create a project FILE with .htm extension that contains
            %directories for the .mat structure files
            proj{1} = [dFolder,'\RegionData.mat'];
            proj{2} = [dFolder,'\RegionConfig.mat'];
            proj{3} =[dFolder,'\InputData.mat'];
            FID = fopen([dProjLoc,dProjFile],'w');
                fprintf(FID,'%s\n',proj{:});
            fclose(FID);
            projSaved = true;
    ...
    function cbFC(hObject,evt)
        %Close
        if projSaved == false
            A = questdlg('Closing window. Save before quitting?');
            if strcmp(A,'Yes') == true
                cbFSM;
                close();
            elseif strcmp(A,'No') == true
                close();
            end
        else
            close();
...
%DATA MENU CALLBACKS
    ...
    function cbSC(hObject,evt)
        ...
        dataFilePath = [dFileLoc dFileName];
        region_config(1).lastDir = dFileLoc;
...
%VIEW MENU CALLBACKS
    ...
    function cbVRO(hObject,evt)
        ...
        region_data = guidata(h.fig);
        for i = 1:hierarchy_regions
            show_active_columns(region_config(i),region_data(i).activeColumns,region_data(i).prediction,region_config(i).seq_time);
        end
