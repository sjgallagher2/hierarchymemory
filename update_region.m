...
function [columns, cells, prediction,n, output,activeColumns] = update_region(columns,cells,segments,data,n,synThreshold,...
    ...
    minOverlapDuty, minOverlap, LearningRadius,segment,t,queue)
    ...
    if temporal_memory == true
            ...
            if isempty( find(activeColumns == i) )
                ...
                for j = 1:n.cellpercol
                    ...
                    if t > 1
                        if mycell.state(t-1) == 2 %if cell was predicting, and is now in an active column
                            ...
                            %%%%%%%%%%%%%%%   LEARNING CELL
                            mycell.learn(t) = false;
                            %A cell that wasn't predicting is not the
                            %learning cell
                            mycell.state(t) = 0;
                ...
                if columns(i).active_cell == -1
                    ...
                    for j = 1:n.cellpercol
                        ...
                        %%%%%%%%%%%%%%%   LEARNING CELL
                        mycell.learn(t) = false;
                        %No cells in a bursting column are the learning
                        %cell yet
                ...
                if t > 1
                    if columns(i).learning_cell == -1
                        ...
                        columns(i).learning_cell = randi(n.cellpercol,1,1);
                        ...
                        %%%%%%%%%%%%%%%   LEARNING CELL
                        mycell.learn(t) = true;
                        %This learning cell was selected at random
                        
                        
                        
                        fprintf('>Cell %d in column %d randomly chosen as learning cell.\n',mycell.layer,mycell.col)
                        ...
                        mycell.segs( mycell.nseg ).index = 1;
                        %give that cell a segment with the active cells from
                        %the previous timestep.
                        [mycell.segs( mycell.nseg ).locations mycell.segs( mycell.nseg ).perm mycell.segs( mycell.nseg ).synCon] = make_distal_segment(LearningRadius,synThreshold,n,i,columns(i).learning_cell, cells, t);
                        cells( loc ) = mycell;
                    end
                end
        ...
        fprintf('Finding predictive cells (t=%d)...\n',t)
        for i = 1:n.cells
            ...
            if activeSeg > 0
                ...
                for fifi = 1:numel(cells(i).segs(activeSeg).locations)
                    foo = cells( cells(i).segs(activeSeg).locations(fifi) ).col;
                    fprintf('>it is connected to column %d.\n',foo)
                end
        ...
        %Once synapse permanences are updated, all synapses are checked for
        %connection.
        if t > 1
            fprintf('Updating synapses...\n')
            for i = 1:n.cells
    ...
    if n.cols > 0   %If columns were created
        ...
        for i = 1:n.cells
            if cells(i).state(t) == 2 %if the cell is predicting
                %follow its active segment
                for s = 1:cells(i).nseg
                    if cells(i).segs(s).active == 1
                        for mu = 1:numel(cells(i).segs(s).locations)
                            %for each location, place the location to
                            %prediction array
                            if cells(i).segs(s).synCon == 1
                                prediction_loc = [prediction_loc, cells( mu ).col];
                            end
                        end
                    end
                end
